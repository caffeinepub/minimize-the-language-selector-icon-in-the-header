...
import { BlogPost, ContactSubmission, EmailCapture, AdminSettings, UserRole, UserProfile, InstagramFeedItem, ShopProduct, TravelStyle, TravelStyleUpdate } from '../backend';
...
export function useGetPublishedBlogsPaginated(offset: number, limit: number) {
  const { actor, isFetching } = useActor();

  return useQuery<BlogPost[]>({
    queryKey: ['publishedBlogsPaginated', offset, limit],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getPublishedBlogsPaginated(BigInt(offset), BigInt(limit));
    },
    enabled: !!actor && !isFetching,
    // Automatically refetch every 60 seconds to check for newly published scheduled posts
    refetchInterval: 60000, // 60 seconds
...
export function useGetAllTravelStyles() {
  const { actor, isFetching } = useActor();

  return useQuery<TravelStyle[]>({
    queryKey: ['allTravelStyles'],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getAllTravelStyles();
    },
    enabled: !!actor && !isFetching,
  });
}

export function useUpdateTravelStyle() {
  const { actor } = useActor();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (update: TravelStyleUpdate) => {
      if (!actor) throw new Error('Actor not available');
      return actor.updateTravelStyle(update);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['allTravelStyles'] });
      showToast('Travel style updated successfully!', 'success');
    },
    onError: (error: any) => {
      showToast(error.message || 'Failed to update travel style', 'error');
    },
  });
}
