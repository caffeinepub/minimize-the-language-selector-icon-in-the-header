...
actor {
    ...
    public type TravelStyleUpdate = {
        name : Text;
        description : Text;
        baseImage : Text;
    };

    var travelStyles = textMap.empty<TravelStyle>();

    public shared ({ caller }) func addTravelStyle(travelStyle : TravelStyle) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can add travel styles");
        };
        travelStyles := textMap.put(travelStyles, travelStyle.id, travelStyle);
    };

    public query func getAllTravelStyles() : async [TravelStyle] {
        Iter.toArray(textMap.vals(travelStyles));
    };

    public shared ({ caller }) func updateTravelStyle(update : TravelStyleUpdate) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can update travel styles");
        };

        var found = false;

        for ((id, travelStyle) in textMap.entries(travelStyles)) {
            if (update.name == travelStyle.name and update.description == travelStyle.description) {
                let updatedTravelStyle = { travelStyle with baseImage = ?update.baseImage };
                travelStyles := textMap.put(travelStyles, id, updatedTravelStyle);
                found := true;
            };
        };

        if (not found) {
            Debug.trap("Travel style not found");
        };
    };

    public shared ({ caller }) func deleteTravelStyle(id : Text) : async () {
        if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
            Debug.trap("Unauthorized: Only admins can delete travel styles");
        };
        travelStyles := textMap.delete(travelStyles, id);
    };
