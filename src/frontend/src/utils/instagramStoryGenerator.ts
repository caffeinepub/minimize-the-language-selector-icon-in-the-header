...
const drawTextContent = (
  ...
  ctx.font = 'bold 64px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
  ctx.fillStyle = BRAND_COLORS.primary;
  ...
  ctx.font = '36px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
  ...
  // Travel message - perfectly centered
  ctx.font = 'bold 42px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
  ctx.fillStyle = BRAND_COLORS.primary;
  ...
  // Brand footer - perfectly centered
  currentY = height - 200;
  ctx.font = 'bold 48px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
  ctx.fillStyle = BRAND_COLORS.primary;
  ctx.fillText('Travel Butts', width / 2, currentY);
  
  currentY += 60;
  ctx.font = '28px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
  ctx.fillStyle = config.backgroundType === 'custom' ? BRAND_COLORS.primary : BRAND_COLORS.neutralLight;
  ctx.fillText('Pack smart, travel light, explore more.', width / 2, currentY);
  
  currentY += 50;
  ctx.font = '24px "Gotham", "Montserrat", "Inter", "Helvetica", sans-serif';
...
export const generateGeographyQuizStory = async (score: number, totalQuestions: number, logoUrl?: string): Promise<string> => {
  ...
  // Try to get custom background image
  const customBackgroundUrl = await getCustomStoryBackground();
...
// Helper function to get custom story background
const getCustomStoryBackground = async (): Promise<string | undefined> => {
  try {
    // This would typically use the useFileUrl hook, but since we're in a utility function,
    // we'll need to fetch it directly or pass it as a parameter
    // For now, we'll return undefined and let the calling code handle it
    return undefined;
  } catch (error) {
    console.warn('Could not load custom story background:', error);
    return undefined;
  }
};
